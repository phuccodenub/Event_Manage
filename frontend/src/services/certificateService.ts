...
const certificateService = {
   ...
   */
  verifyCertificateEligibility: async (eventId: string, userId: string): Promise<CertificateVerificationResult> => {
    try {
      const response = await apiClient.get(`/certificates/verify/${eventId}/${userId}`);
   ...
   */
  getCertificateUrl: (eventId: string, userId: string): string => {
    return `${apiClient.defaults.baseURL}/certificates/${eventId}/${userId}`;
  }
};
