      ...
      participation: events
        ...
        .map(event => {
          ...
          return {
            ...
            department: event.community ? `${event.community.name} (${event.department?.name})` : event.department?.name || 'Chưa phân khoa',
            status: event.status
          };
        })
    };
    ...
    const fetchData = async () => {
      try {
        ...
        const [users, events, announcements] = await Promise.all([
          ...
          eventService.getAllEvents({ populate: 'community,department' }),
